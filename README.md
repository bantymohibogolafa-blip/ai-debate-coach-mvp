...
XIAOMI_TTS_MODEL=mimo-v2.5-tts-voicedesign
XIAOMI_TTS_VOICE=
PORT=3001
```
...
检查 Render 或 `server/.env` 是否已经配置 `XIAOMI_TTS_API_KEY`。默认模型使用 `mimo-v2.5-tts-voicedesign`；如需覆盖，可配置 `XIAOMI_TTS_MODEL`。`XIAOMI_TTS_API_URL` 推荐填写小米控制台给出的 OpenAI 兼容 `BASE_URL`：`https://api.xiaomimimo.com/v1`，后端会自动派生 `/audio/speech` 和 `/chat/completions`。`XIAOMI_TTS_VOICE` 可以先留空，后端会使用林婉音色描述；如果小米控制台明确给了可用 voice 值，再填入该值。
