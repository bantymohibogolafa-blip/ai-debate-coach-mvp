...
XIAOMI_TTS_API_URL=https://api.xiaomimimo.com/v1/chat/completions
...
检查 Render 或 `server/.env` 是否已经配置 `XIAOMI_TTS_API_KEY`。默认模型使用 `mimo-v2.5-tts-voicedesign`；如需覆盖，可配置 `XIAOMI_TTS_MODEL`。`XIAOMI_TTS_VOICE` 可以先留空，后端会使用音色描述参数；如果小米控制台明确给了可用 voice 值，再填入该值。如小米 TTS 接口地址不同，也要同步配置 `XIAOMI_TTS_API_URL`。
