...
export const abilityDimensions = [
  { key: 'logic', label: '逻辑推进', weight: 3 / 10 },
  { key: 'defenseStability', label: '防守稳定', weight: 3 / 20 },
  { key: 'counterPressure', label: '反压能力', weight: 1 / 5 },
  { key: 'battlefieldControl', label: '战场控制', weight: 3 / 20 },
  { key: 'expression', label: '表达效率', weight: 1 / 5 }
];
...
// Each review subdimension can feed at most three unified abilities. Shares are
...
function normalizeAbilityRecord(record = {}) {
  ...
  return {
    ...
    coveredDimensions,
    ...versionMetadata,
    appliedProjectionVersion: CURRENT_PROJECTION_VERSION,
...
function buildAbilityHistorySource(record = {}) {
  return {
    ...
    appliedProjectionVersion: record.appliedProjectionVersion || '',
